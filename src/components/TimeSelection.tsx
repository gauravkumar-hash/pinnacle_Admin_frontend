...
interface TimeInterval {
  ...
  max_appointments_per_session?: number; // max appointments per time slot
}
...
const TimeBlock: React.FC<TimeBlockProps> = ({
  ...
  const height =
    ((interval.end - interval.start) / (HOURS_IN_DAY * MINUTES_IN_HOUR)) *
    GRID_HEIGHT;
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      const deltaMinutes =
        (deltaY / GRID_HEIGHT) * (HOURS_IN_DAY * MINUTES_IN_HOUR);
      ...
      if (isDragging) {
        const newStart = Math.max(
          ...
          Math.min(
            ...
            HOURS_IN_DAY * MINUTES_IN_HOUR -
              (dragStart.endTime - dragStart.startTime),
          ),
        );
        const newEnd = newStart + (dragStart.endTime - dragStart.startTime);
        onUpdate(
          interval.id,
          Math.round(newStart / 15) * 15,
          Math.round(newEnd / 15) * 15,
        );
      } else if (isResizing === "top") {
        const newStart = Math.max(
          0,
          Math.min(dragStart.startTime + deltaMinutes, dragStart.endTime - 15),
        );
        ...
        const newEnd = Math.max(
          ...
          Math.min(
            dragStart.endTime + deltaMinutes,
            HOURS_IN_DAY * MINUTES_IN_HOUR,
          ),
        );
      ...
      <div className="px-2 py-1 text-white text-xs font-medium flex items-center justify-between h-full">
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs opacity-90">
            Cutoff: {interval.cutoff_time}m
          </div>
          {interval.max_appointments_per_session && (
            <div className="text-xs opacity-90">
              Max: {interval.max_appointments_per_session}
            </div>
          )}
        </div>
...
const DayColumn: React.FC<DayColumnProps> = ({
  ...
  const handleColumnClick = (e: React.MouseEvent) => {
    ...
    const clickedMinutes =
      Math.round(((y / GRID_HEIGHT) * (HOURS_IN_DAY * MINUTES_IN_HOUR)) / 15) *
      15;

    // Check if click is on an existing interval
    const clickedInterval = intervals.find(
      (interval) =>
        clickedMinutes >= interval.start && clickedMinutes <= interval.end,
    );
    ...
    if (!clickedInterval) {
      ...
      onAddInterval(
        Math.max(
          0,
          Math.min(clickedMinutes, (HOURS_IN_DAY - 1) * MINUTES_IN_HOUR),
        ),
      );
    }
  };
...
const IntervalFormModal: React.FC<IntervalFormModalProps> = ({
    ...
    if (open && interval) {
      ...
      form.setFieldsValue({
        ...
        max_appointments_per_session:
          interval.max_appointments_per_session || undefined,
      });
    }
  }, [open, interval, form]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (
        !timePattern.test(values.start_time) ||
        !timePattern.test(values.end_time)
      ) {
        form.setFields([
          {
            name: "general",
            errors: ["Please enter valid time in HH:MM format"],
          },
        ]);
        return;
      }
      ...
      if (startMinutes >= endMinutes) {
        form.setFields([
          { name: "general", errors: ["End time must be after start time"] },
        ]);
        return;
      }
      ...
      onSave({
        ...
        max_appointments_per_session:
          values.max_appointments_per_session || null,
      });
      form.resetFields();
    } catch (error) {
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical">
        ...
        <Form.Item
          name="max_appointments_per_session"
          label="Max Appointments Per Session"
          help="Maximum number of appointments allowed in this time slot"
          rules={[{ required: false }]}
        >
          <InputNumber
            min={1}
...
export default function TimeSelection({
  ...
  const updateInterval = useCallback(
    (day: string, id: string, start: number, end: number) => {
  ...
  const handleModalSave = useCallback(
    (values: any) => {
      ...
      const startMinutes = values.start_time
        .split(":")
        .reduce((acc: number, time: string) => 60 * acc + +time, 0);
      const endMinutes = values.end_time
        .split(":")
        .reduce((acc: number, time: string) => 60 * acc + +time, 0);
      ...
      if (modalState.isNew) {
        setOperatingHours((prev) => ({
          ...prev,
          [modalState.day!]: [...prev[modalState.day!], updatedInterval].sort(
            (a, b) => a.start - b.start,
          ),
        }));
      } else {
        setOperatingHours((prev) => ({
          ...
          [modalState.day!]: prev[modalState.day!].map((interval) =>
            interval.id === modalState.interval!.id
              ? updatedInterval
              : interval,
          ),
        }));
  ...
  return (
    <>
      <div className="text-center mb-4">
        <Title level={3}>
          {branchName
            ? `${branchName} - Operating Hours`
            : "Clinic Operating Hours"}
        </Title>
        <Text type="secondary">
          Click on any time slot to add operating hours. Drag to move, resize
          from edges, or delete intervals.
        </Text>
      </div>
        ...
        <div className="space-y-4">
          ...
          <Card type="inner" size="small">
            <div className="flex flex-wrap items-center gap-3 justify-center">
              ...
              >
                Copy {sourceDay === "PH" ? "PH" : sourceDay.slice(0, 3)} to{" "}
                {destinationDay === "All"
                  ? "All"
                  : destinationDay === "PH"
                    ? "PH"
                    : destinationDay.slice(0, 3)}
              </Button>
            </div>
          </Card>
          ...
          <Card type="inner" size="small">
            <div className="flex flex-wrap items-center gap-3 justify-center">
              ...
              <Button danger size="small" onClick={handleClearOperation}>
          ...
          <div className="ml-12 grid grid-cols-1 md:grid-cols-4 lg:grid-cols-8 gap-2">
            {DAYS.map((day) => (
              <DayColumn
                ...
                onUpdateInterval={(id, start, end) =>
                  updateInterval(day, id, start, end)
                }
