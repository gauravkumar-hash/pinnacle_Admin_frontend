...
import { Layout, Typography, Table, Tag, Select, Space, Modal, Form, Input, Button, message, Descriptions, Avatar, Badge } from 'antd';
import { EyeOutlined, EditOutlined, UserOutlined } from '@ant-design/icons';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { useAuth } from '../../context/AuthProvider';
import { getAppointmentRequests, updateRequestStatus, getSpecialists, AppointmentRequest } from '../../apis/specialist-care';
...
export const AppointmentRequestsScreen = () => {
    ...
    const [specialistFilter, setSpecialistFilter] = useState<number | undefined>();
    ...
    const { data: requests = [], isLoading } = useQuery({
        ...
        queryFn: () => getAppointmentRequests(session!, onError, {
            status: statusFilter,
    ...
    const openStatusModal = (record: AppointmentRequest) => {
        ...
        form.setFieldsValue({ status: record.status, status_message: record.status_message });
    };
    ...
    const columns = [
        ...
        {
            title: 'Specialist',
            render: (_: any, r: AppointmentRequest) => r.specialist ? (
                <Space>
        ...
        },
        {
            title: 'Status',
            dataIndex: 'status',
            render: (v: string) => <Badge color={STATUS_COLOR[v]} text={<Tag color={STATUS_COLOR[v]}>{v.toUpperCase()}</Tag>} />,
        },
        {
            title: 'Message',
        ...
        },
        {
            ...
            render: (_: any, record: AppointmentRequest) => (
                <Space>
                    <Button icon={<EyeOutlined />} size="small" onClick={() => setViewRecord(record)}>View</Button>
                    <Button icon={<EditOutlined />} size="small" type="primary" onClick={() => openStatusModal(record)}>Update Status</Button>
                </Space>
            ),
        },
    ];
        ...
        <Layout style={{ padding: '24px', background: '#fff', flex: 1 }}>
            ...
            <Content>
                <div className="flex justify-between items-center mb-4">
                    <Title level={4} style={{ margin: 0 }}>Appointment Requests</Title>
                    <Space>
                        <Select
                            allowClear
                ...
                <Modal
                    ...
                    footer={[
                        ...
                        <Button key="update" type="primary" onClick={() => { setViewRecord(null); openStatusModal(viewRecord!); }}>
                            Update Status
                        </Button>
                    ]}
                    width={600}
                >
                    {viewRecord && (
                        <Descriptions bordered column={1} size="small">
                            ...
                            <Descriptions.Item label="Date of Birth">{viewRecord.patient_dob || '—'}</Descriptions.Item>
                            ...
                            <Descriptions.Item label="Specialist">
                                {viewRecord.specialist ? `${viewRecord.specialist.title} ${viewRecord.specialist.name}` : '—'}
                            </Descriptions.Item>
                            <Descriptions.Item label="Preferred Days">{viewRecord.preferred_days || '—'}</Descriptions.Item>
                            <Descriptions.Item label="Preferred Time">{viewRecord.preferred_time || '—'}</Descriptions.Item>
                            ...
                            <Descriptions.Item label="Status Message">{viewRecord.status_message || '—'}</Descriptions.Item>
