...
import { CorporateCodeQuotaDashboard } from "../../components/CorporateCodeQuotaDashboard";
...
export default function AppointmentCorporateCodes() {
  ...
  const [editingRecord, setEditingRecord] =
    useState<AppointmentCorporateCode | null>(null);
  ...
  const [surveyEditorRecord, setSurveyEditorRecord] =
    useState<AppointmentCorporateCode | null>(null);
  const [quotaDrawerVisible, setQuotaDrawerVisible] = useState(false);
  const [quotaDrawerRecord, setQuotaDrawerRecord] =
    useState<AppointmentCorporateCode | null>(null);
  ...
  const exportSurveyMutation = useDownload(
    (params: { codeId: string }) =>
      `/api/admin/appointments/v1/corporate-codes/${params.codeId}/export-survey-csv`,
  ...
  const {
    data: corporateCodesData,
    isLoading: corporateCodesLoading,
    error: corporateCodesError,
  } = useQuery({
    ...
    form.setFieldsValue({
      ...
      validity_period:
        record.valid_from && record.valid_to
          ? [dayjs(record.valid_from), dayjs(record.valid_to)]
          : null,
      patient_survey: record.patient_survey || {},
      only_primary_user: record.only_primary_user || false,
      service_group_ids:
        record.service_groups?.map((sg) => (sg as any).id) || [],
      onsite_branches:
        record.onsite_branches?.map((branch) => ({
          ...branch,
          date_range: [
            dayjs((branch as any).start_date),
            dayjs((branch as any).end_date),
          ],
        })) || [],
    });
    setModalVisible(true);
  ...
  const handleDelete = (codeId: string) => {
    Modal.confirm({
      ...
      content:
        "Are you sure you want to delete this corporate code? This action cannot be undone.",
      okText: "Yes, Delete",
      okType: "danger",
      cancelText: "Cancel",
      onOk: () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const requestBody = {
        ...
        max_appointments_total: values.max_appointments_total || null,
        max_appointments_per_day: values.max_appointments_per_day || null,
        service_group_ids: values.service_group_ids || [],
        onsite_branches:
          values.onsite_branches?.map((branch: any) => ({
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: AppointmentCorporateCode) => {
        const groupNames =
          record.service_groups?.map((sg) => (sg as any).name) || [];
    ...
    },
    {
      ...
      render: (_: any, record: AppointmentCorporateCode) => {
        const patientOptions =
          (record.patient_survey?.patient as string[]) || [];
    ...
    },
    {
      ...
      render: (_: any, record: AppointmentCorporateCode) => (
        <Space>
          ...
          {record.only_primary_user && <Tag color="purple">Primary Only</Tag>}
        </Space>
      ),
    },
    {
  ...
  return (
    <ErrorBoundary>
      <div className="p-6 w-full">
        ...
        {(createCorporateCodeMutation.isError ||
          updateCorporateCodeMutation.isError ||
          deleteCorporateCodeMutation.isError) && (
          <ErrorAlert
            error={
              (createCorporateCodeMutation.error ||
                updateCorporateCodeMutation.error ||
                deleteCorporateCodeMutation.error) as any
            }
            onDismiss={() => {
        ...
        {corporateCodesError ? (
          ...
          <Card>
            <div className="text-center p-8">
              <div className="text-lg font-medium mb-2">
                No corporate codes configured
              </div>
              <div className="text-gray-500 mb-4">
                Corporate codes allow organizations to book appointments with
                special terms.
              </div>
              <Button
                type="primary"
          ...
          <Table
            ...
            loading={
              loading ||
              createCorporateCodeMutation.isPending ||
              updateCorporateCodeMutation.isPending ||
              deleteCorporateCodeMutation.isPending
            }
            pagination={{
              pageSize: 20,
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              <Form.Item
                label="Total Appointment Quota"
                name="max_appointments_total"
                help="Maximum total appointments allowed"
              >
                <Input
                  type="number"
                  min={0}
              ...
              <Form.Item
                label="Daily Appointment Quota"
                name="max_appointments_per_day"
                help="Maximum appointments per day"
              >
                <Input
                  type="number"
                  min={0}
                  ...
                  <>
                      ...
                      <div key={key} className="flex gap-2 mb-2">
                        <Form.Item
                          ...
                          rules={[
                            {
                              required: true,
                              message: "Please enter survey option",
                            },
                          ]}
                        >
                    ...
                    {fields.length === 0 && (
                      <div className="text-gray-500 text-sm mb-2">
                        No patient survey options defined. Click "Add Option" to
                        create options.
                      </div>
                    )}
                  </>
            ...
            <Space direction="vertical" className="w-full">
              ...
              >
                <Checkbox>
                  Only Primary User (Restrict bookings to primary user only)
                </Checkbox>
              </Form.Item>
            </Space>
          </Form>
        ...
        {/* Quota Usage Drawer */}
        <Drawer
          title="Appointment Quota Usage"
          placement="right"
          width={600}
          open={quotaDrawerVisible}
          onClose={() => {
            setQuotaDrawerVisible(false);
            setQuotaDrawerRecord(null);
          }}
        >
          {quotaDrawerRecord && (
            <CorporateCodeQuotaDashboard
              codeId={quotaDrawerRecord.id}
              codeName={quotaDrawerRecord.code}
            />
          )}
        </Drawer>
      </div>
